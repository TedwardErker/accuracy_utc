...
#+PROPERTY: header-args:R :cache no :results output :exports both :tangle yes :session *R*
...
------------
* COMMENT to run on krusty
set R directory to
/ssh:krusty:~/accuracy_utc/code/
* Pre [2016-11-11 Fri]
...
** TODO functions REWRITE FUNCTIONS


I need to rewrite the functions to calculate the error that I would
see from every source of error:
- horizontal shift
- lean
- rasterization (1m resolution)  - this is like doing a regular sample!
- classification
-
...
#+begin_src R
  ...
  pct.tree.shifted <- lapply(circles, function(circle) {
       values <- extract(pseudo.classification$tree.raster, circle) %>% unlist()
       pct.tree.shifted <- sum(values == 1, na.rm = T)/length(values)
       pct.tree.shifted
   }
   ) %>%
       unlist()
   ...
   pct.tree.true <- lapply(circles, function(circle) {
       true.tree.cp <- crop(true.tree.cover.realization$tree.cover.sp, circle)
       if(is.null(true.tree.cp)) {
           true.tree.cp.area <- 0
       } else {
           true.tree.cp.area <- gArea(true.tree.cp)
       }
       pct.tree.true <- true.tree.cp.area/gArea(circle)
   }) %>%
       unlist()

   agreement.df <- data.frame(pct.tree.true = pct.tree.true,
                              pct.tree.classification = pct.tree.classification,
                              pct.tree.shifted = pct.tree.shifted,
                              class.and.shift.error = abs(pct.tree.true - pct.tree.classification),
                              shift.error = abs(pct.tree.true - pct.tree.shifted),
                              class.error = abs(pct.tree.classification - pct.tree.shifted),
                              radius = radii,
   ...
   return(list(agreement.df = agreement.df, true.tree.cover.realization = true.tree.cover.realization, pseudo.classification = pseudo.classification, circles = circles))
   }
...
#+begin_src R
  ...
  #  cl <- makeCluster(detectCores())
    ...
    replication <- foreach(j = 1:300, .packages = c("plyr","dplyr","rgeos","sp","doParallel","foreach","raster")) %dopar% {
        make.class.trueCover.agreement.df(replicate = j)
    }

    closeAllConnections()

  saveRDS(replication, file = "../DD/replication_exact_results.rds")
#+end_src

#+RESULTS:

#+begin_src R
replication <- readRDS("../DD/replication_exact_results.rds")
...
#+begin_src R :exports results :results graphics :file ../figs/class.and.shift.errorVSarea_TrueAndExactClass_lines.png
ggplot(dfs, aes(x = area, y = class.and.shift.error,  group = replicate)) + geom_line(alpha = .1)
#+end_src

#+RESULTS:
[[file:../figs/class.and.shift.errorVSarea_TrueAndExactClass_lines.png]]

#+begin_src R :exports results :results graphics :file ../figs/just.shift.errorVSarea_TrueAndExactClass_lines.png
ggplot(dfs, aes(x = area, y = shift.error,  group = replicate)) + geom_line(alpha = .1)
#+end_src

#+RESULTS:
[[file:../figs/just.shift.errorVSarea_TrueAndExactClass_lines.png]]

#+begin_src R :exports results :results graphics :file ../figs/class.to.shift.errorVSarea_TrueAndExactClass_lines.png
ggplot(dfs, aes(x = area, y = class.error,  group = replicate)) + geom_line(alpha = .1)
#+end_src

#+RESULTS:
[[file:../figs/class.to.shift.errorVSarea_TrueAndExactClass_lines.png]]

#+begin_src R :exports results :results graphics :file ../figs/class.to.shift.errorVSarea_TrueAndExactClass.png
ggplot(dfs, aes(x = area, y = class.error, group = area)) + geom_boxplot()
#+end_src

#+RESULTS:
[[file:../figs/class.to.shift.errorVSarea_TrueAndExactClass.png]]


#+begin_src R
  library(tidyr)
        sum.df <- dfs %>%
          select(class.and.shift.error, class.error, shift.error,area) %>%
          gather(key = error.type, value = error, -area) %>%
          group_by(error.type,area) %>%
...
Groups: error.type [?]
              ...
              error.type  area error_mean   error_sd
                   <chr> <dbl>      <dbl>      <dbl>
1  class.and.shift.error     3 0.20954003 0.30201641
2  class.and.shift.error    50 0.14411123 0.14491834
3  class.and.shift.error   154 0.10518739 0.09065990
4  class.and.shift.error   314 0.07209668 0.06475537
5  class.and.shift.error   531 0.05068896 0.04700505
6  class.and.shift.error   804 0.03816162 0.03610903
7  class.and.shift.error  1134 0.03113443 0.02591014
8  class.and.shift.error  1521 0.02648508 0.02221413
9  class.and.shift.error  1963 0.02366555 0.01923211
10 class.and.shift.error  2463 0.02115712 0.01677489
# ... with 41 more rows
#+end_example

#+begin_src R :exports results :results graphics :file ../figs/mean_sd_errors_exact.png
limits <- aes(ymax = error_mean + error_sd, ymin=error_mean - error_sd)

ggplot(sum.df, aes(x = area, y = error_mean, color = error.type)) + geom_point() + geom_pointrange(limits)
#+end_src

#+RESULTS:
[[file:../figs/mean_sd_errors_exact.png]]
...
#+begin_src R :results none :eval no
  dir.create(path = "../figs/agreement_plots_exactestimate")
    ...
    lapply(replication, function(rep) {
        ...
        lapply(1:length(rep$circles), function(i) {
            circle <- rep$circles[[i]]
            circle.fort <- fortify(circle) %>%
                rename(x = long, y = lat)
            circle.plot <- geom_polygon(data = circle.fort, aes(x = x, y = y, group = group), color = "black", fill = NA)

            agreement <- rep$agreement.df[i,] %>%
                mutate_all(.funs = round, digits = 3)

            agreement.label.df <- data.frame(x = c(-15,-15,65,70),
                                          y = c(102,90,102,90),
                                          label = c(paste0("True Tree Cover: ",agreement$pct.tree.true*100,"%"),
                                                    paste0("Map Tree Cover: ",agreement$pct.tree.classification*100,"%"),
                                                    paste0("Error: ",agreement$error*100,"%"),
                                                    paste0("Area: ",agreement$area)))

            agreement.label <- geom_label(data = agreement.label.df, aes(x = x, y = y, label = label), size = 7, hjust = 0)
            ...
            out.plot <- ggplot() +
                ...
                utc.col +
                agreement.label +
                ggtitle(label = paste0("replicate #: ",agreement$replicate)) +
                theme(plot.title = element_text(size = 30))

            out.plot


            ggsave(filename = sprintf("../figs/agreement_plots_exactestimate/Rep-%02s_Area-%05s.png",agreement$replicate,agreement$area),
                   type = "cairo-png")
                       })
...
#+BEGIN_SRC sh
  convert -delay 100 ../figs/agreement_plots_exactestimate/*.png -loop 0 ../figs/agreement_plots_exactestimate/AgreementExact.gif
#+END_SRC

#+RESULTS[22634961e7ff0fe77df32d27487dea458c05403f]:
: convert -delay 100
