...
#+begin_src R
  ...
  ggplot() + geom_polygon(data = tree.sp.fort[tree.sp.fort$id %in% tree.sp.fort[tree.sp.fort$hole,]$id,],fill = "#7fc97f", aes(group = group, x = x, y = y))
...
#+begin_src R
  ...
  true.tree <-  geom_polygon(data = tree.sp.fort[tree.sp.fort$id %in% tree.sp.fort[tree.sp.fort$hole,]$id,],fill = NA, color = "#33a02c", aes(group = group, x = x, y = y))
    ...
    p2 <- p1 +
        geom_polygon(data = tree.sp.fort.shift[tree.sp.fort.shift$id %in% tree.sp.fort.shift[tree.sp.fort.shift$hole,]$id,],fill = "#b2df8a", aes(group = group, x = x, y = y), size = 3)
...
#+RESULTS:
:  Error in rename(., x = long, y = lat) : unused argument (y = lat)
...
#+begin_src R
  ...
  r.plot <- ggplot(tree.r.shift.df, aes(x = x, y = y, fill = layer))  + geom_raster() + theme_bw() +
      scale_fill_continuous(high = "green", na.value = "white")
...
#+begin_src R
  ...
  bound <- boundaries(tree.r.shift)

  tree.r.shift[is.na(tree.r.shift)] <- 0

  tree <- stack(tree.r.shift, bound)

  rand <- runif(ncell(tree))

  interior.error.rate <- .10
  additional.boundary.error.rate <- .15


  atree.r.shift <- tree.r.shift - interior.error.rate
  bound[bound == 1] <- additional.boundary.error.rate

  btree.r.shift <- atree.r.shift - bound

  rand.raster <- raster(btree.r.shift)
  values(rand.raster) <- runif(ncell(rand.raster))

  classified.tree <- overlay(btree.r.shift, rand.raster, fun=function(x,y){ifelse(x>y, T,F)})
  plot(classified.tree)


  classified.tree.df <- as.data.frame(classified.tree, xy = T)

  ggplot(classified.tree.df, aes(x = x, y = y, fill = layer)) + geom_raster()
...
#+begin_src R
  ...
  for(i in 1:length(radii)) {
      ...
      if (i > 1) {
          ...
          pts.sub <- sample(pts.list[[i-1]], size = n*area.ratio.small.large)
...
#+RESULTS:
: Error in sample.Spatial(as(x, "Spatial"), n_tot * (1 + its * 0.1), type = type,  :
:   sampling type Regular not recognized
